...
R1_patterns = {
    ...
    'hBC': re.compile('AACACC(.*)ATAGCAAG'),
    # BC1 is from the start to the start of the W1 sequence
    'BC1': re.compile('^(.*)ATAGCAAGTT'),
...
R2_patterns = {
    ...
    'BC2_UMI': re.compile('GCCCGAATTC(.{1,10})'), # 1,14 means 1 to 14 characters
...
Combined.to_csv("./output/"+filename+".csv")
